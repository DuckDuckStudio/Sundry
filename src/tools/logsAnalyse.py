...
def main(args: list[str]) -> Literal[0, 1]:
    """
    `sundry ga-logs-analyse` 命令的入口函数。

    Args:
        args:
            传入的必须参数列表
            - `sundry ga-logs-analyse <PR 链接>`
            - `sundry ga-logs-analyse <PR 编号>`
            - `sundry ga-logs-analyse <检查链接>`

            传入的可选参数列表
            - `sundry ga-logs-analyse <...> [是否启用详细模式]`
    ...
    log_zip_file_path = get_validation_log_zip_from_args(args)
    ...
    detailed = (args[1] in YES) if (len(args) == 2) else False  # 默认为不启用详细模式

    found = analyze_validation_result((logs_dir_path / "ValidationResult"), detailed)
    found = analyze_installation_verification_logs((logs_dir_path / "InstallationVerificationLogs"), detailed) or found

    if not found:
        print(f"{消息头.警告} 未找到可能的问题")
        if not detailed:
            print(f'{消息头.提示} 请尝试使用 {Fore.BLUE}sundry ga-logs-analyse "{args[0]}" y{Fore.RESET} 来查看一般错误/异常')
    ...
    if exit_codes_io:
        ...
        for row in reader:
            if exit_code in [row["Hex"], row["Dec"], row["InvDec"], row["Symbol"]]:
                ...
                print(f"{消息头.提示} {" | ".join([f"Hex: {Fore.BLUE}{row['Hex']}{Fore.RESET}", f"Dec: {Fore.BLUE}{row['Dec']}{Fore.RESET}", f"InvDec: {Fore.BLUE}{row['InvDec']}{Fore.RESET}", f"Symbol: {Fore.BLUE}{row['Symbol']}{Fore.RESET}", f"Description: {Fore.BLUE}{row['Description']}{Fore.RESET}"]).replace(f"{Fore.BLUE}{exit_code}{Fore.RESET}", f"{Fore.MAGENTA}{exit_code}{Fore.RESET}")}")  # pylint: disable=line-too-long / C0301
