...
def 获取用户输入(键: str):
    ...
    提示消息 = {
        "路径": {
            "winget-pkgs": f"{消息头.问题}您的本地 winget-{Fore.YELLOW}pkgs{Fore.RESET} 仓库在哪里: ",
            "winget-tools": f"{消息头.问题}您的本地 winget-{Fore.YELLOW}tools{Fore.RESET} 仓库在哪里: "
        },
        "仓库": {
            "pkgs-repo": f"{消息头.问题}您的远程 winget-{Fore.YELLOW}pkgs{Fore.RESET} 仓库是什么: ",
            "tools-repo": f"{消息头.问题}您的远程 winget-{Fore.YELLOW}tools{Fore.RESET} 仓库是什么: "
        },
        "签名": {
            "signature": f"{消息头.问题}是否要为 Git 提交签名？(默认为{Fore.YELLOW}否{Fore.RESET}): "
        },
        "i18n": {
            "lang": f"{消息头.问题}你希望 Sundry 使用哪种语言运行？[{Fore.GREEN}zh-CN(默认){Fore.RESET}, en-US]: "
        }
    }
            ...
            if len(parts) == 2:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        print(f"{消息头.错误}指定的仓库不存在或没有权限访问")
                    else:
                        print(f"{消息头.错误}检查仓库失败，状态码: {Fore.YELLOW}{response.status_code}{Fore.RESET}")
                except Exception as e:
                    print(f"{消息头.错误}检查仓库失败:\n{Fore.RED}{e}{Fore.RESET}")
            else:
                print(f"{消息头.错误}请输入正确的仓库格式，例如: owner/repo")
    # 签名输入
    elif 键 in 提示消息["签名"]:
    ...
    # i18n输入
        ...
        while True:
            ...
            if 答案 not in ["", "zh-cn", "en-us"]:
                print(f"{消息头.错误}不支持的语言，请在 [] 中的语言中选择一种")
            else:
                语言映射 = {
                    "": "zh-CN",
                    "zh-cn": "zh-CN",
        ...
        return input(f"{消息头.问题}请输入 {键} 的值: ")

# 初始化配置文件
def 初始化配置文件(配置文件: str):
    if not os.path.exists(配置文件) or (input(f"{消息头.警告}已经存在了一份配置文件，要覆盖它吗[Y/N]: ").lower() in ["y", "yes", "要", "覆盖", "force"]):
        默认配置 = '''{
    "version": "develop",
    "winget-pkgs": "",
    "winget-tools": "",
...
'''
        ...
        if input(f"{消息头.可选问题}是否要让配置文件兼容 Sundry-old 和 Sundry-Locale (i18n 分支)？(默认为{Fore.YELLOW}否{Fore.RESET}): ").lower() in ["y", "yes", "要", "是", "true"]:
