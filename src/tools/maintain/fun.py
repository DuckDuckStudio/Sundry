...
def 编辑fun() -> int:
    print(f"{MSHead.Information} fun.txt 位于 {FUN_TEXT_PATH}")
    print(f"{MSHead.Information} 尝试打开 fun.txt ...")
...
def 导入fun(原fun文件: str, 导入fun文件: str) -> int:
    ...
    try:
        ...
        with open(导入fun文件, 'r', encoding="utf-8") as file:
            ...
            if not 导入的fun:
                print(f"{MSHead.Warning} {Fore.BLUE}{导入fun文件}{Fore.RESET} (导入) {Fore.YELLOW}为空{Fore.RESET}")
                return 0
    except FileNotFoundError:
        print(f"{MSHead.Error} {Fore.YELLOW}未找到{Fore.RESET} {Fore.BLUE}{导入fun文件}{Fore.RESET} (导入)")
        return 1
    except Exception as e:
        print(f"{MSHead.Error} 读取 {Fore.BLUE}{导入fun文件}{Fore.RESET} (导入) 时发生异常:\n{Fore.RED}{e}{Fore.RESET}")
        return 1
    # 去重但保持原始顺序
    try:
        已有fun: set[str] = set()
        ...
        print(f"{MSHead.Error} 对 {Fore.BLUE}{导入的fun}{Fore.RESET} 去重时发生异常:\n{Fore.RED}{e}{Fore.RESET}")
        return 1
    # 覆盖原fun文件
    try:
        if not os.path.exists(原fun文件):
            print(f"{MSHead.Error} {Fore.YELLOW}未找到{Fore.RESET} {Fore.BLUE}{原fun文件}{Fore.RESET} (原始)")
            # 询问用户是否创建一个
            try:
                input(f"{MSHead.Question} 创建个新的 fun.txt? [ENTER/CTRL+C]")
        ...
        print(f"{MSHead.Success} 已将 {Fore.BLUE}{导入fun文件}{Fore.RESET} 导入到 {Fore.BLUE}{原fun文件}{Fore.RESET}")
        return 0
    except Exception as e:
        print(f"{MSHead.Error} 导入 {Fore.BLUE}{导入fun文件}{Fore.RESET} 到 {Fore.BLUE}{原fun文件}{Fore.RESET} 时发生异常:\n{Fore.RED}{e}{Fore.RESET}")
        return 1
...
def 移除fun(条目: str) -> int:
    try:
        ...
        if not (f"{条目}\n" in lines):
            print(f"{MSHead.Warning} 在 {Fore.BLUE}{FUN_TEXT_PATH}{Fore.RESET} 中{Fore.YELLOW}未找到{Fore.RESET} {Fore.BLUE}{条目.replace("\\n", "\n")}{Fore.RESET}")
            return 0
        while f"{条目}\n" in lines:
            # 移除所有匹配的条目
            lines.remove(f"{条目}\n")
        ...
        print(f"{MSHead.Success} 已将 {Fore.BLUE}{条目.replace("\\n", "\n")}{Fore.RESET} 从 {Fore.BLUE}{FUN_TEXT_PATH}{Fore.RESET} 中移除")
        return 0
    except FileNotFoundError:
        print(f"{MSHead.Error} {Fore.YELLOW}未找到{Fore.RESET} {Fore.BLUE}{FUN_TEXT_PATH}{Fore.RESET}")
        return 1
    except Exception as e:
        print(f"{MSHead.Error} 移除 {Fore.BLUE}{条目.replace("\\n", "\n")}{Fore.RESET} 时发生异常:\n{Fore.RED}{e}{Fore.RESET}")
        return 1
...
def 添加fun(新条目: str) -> int:
    ...
    try:
        ...
        print(f"{MSHead.Error} 读取 {Fore.BLUE}{FUN_TEXT_PATH}{Fore.RESET} 时发生异常:\n{Fore.RED}{e}{Fore.RESET}")
        return 1
    # 添加新条目
    try:
        with open(FUN_TEXT_PATH, 'a', encoding="utf-8") as file:
            file.write(f"{新条目}\n")
        print(f"{MSHead.Success} 已将 {Fore.BLUE}{新条目.replace("\\n", "\n")}{Fore.RESET} 添加进 {Fore.BLUE}{FUN_TEXT_PATH}{Fore.RESET}")
        return 0
    except Exception as e:
        print(f"{MSHead.Error} 添加 {Fore.BLUE}{新条目.replace("\\n", "\n")}{Fore.RESET} 到 {Fore.BLUE}{FUN_TEXT_PATH}{Fore.RESET} 时发生异常:\n{Fore.RED}{e}{Fore.RESET}")
        return 1
            ...
            if content := file.read().strip():
                if 随机:
                    ...
                    if lines:
                        ...
                        print(f"{MSHead.Error} 没有合适的行以供随机")
                else:
                    # 整个文件，不处理 \n
                    print(content)
            else:
                print(f"{MSHead.Warning} {Fore.BLUE}{FUN_TEXT_PATH}{Fore.RESET} {Fore.YELLOW}为空{Fore.RESET}")
        return 0
    except FileNotFoundError:
        print(f"{MSHead.Error} {Fore.YELLOW}未找到{Fore.RESET} {Fore.BLUE}{FUN_TEXT_PATH}{Fore.RESET}")
        return 1
