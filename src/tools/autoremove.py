...
def 检查包版本(包标识符: str, 版本列表: list[str], 跳过检查: bool) -> None:
    ...
    for 版本 in 版本列表:
        ...
        print(f"\n{Fore.BLUE}INFO{Fore.RESET} 正在检查 {Fore.BLUE}{包标识符} {版本}{Fore.RESET} ({版本列表.index(版本)+1}/{len(版本列表)}) ...")
        if not 跳过检查:
...
def 检查所有安装程序URL(包标识符: str, 包版本: str, 在浏览器中打开: bool) -> tuple[int, str]:
    """
    检查指定 包标识符 包版本 的安装程序清单中的所有 InstallerUrl 是否有失效的。
    ...
    返回示例: 3, "失效 (404)"
    """
    清单目录 = 获取清单目录(包标识符, 包版本)
    if not 清单目录:
        raise KeyboardInterrupt
    try:
        # 获取安装程序清单中的所有 InstallerUrl 字段的值
        with open(os.path.join(清单目录, f"{包标识符}.installer.yaml"), "r", encoding="utf-8") as 清单文件:
