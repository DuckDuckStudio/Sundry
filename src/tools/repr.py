...
def repr_file(path: str, encode: str="utf-8"):
    try:
        ...
        print(f"{MSHead.Error} {Fore.RED}没有权限{Fore.RESET}")
        return 1
    except FileNotFoundError:
        print(f"{MSHead.Error} {Fore.RED}文件不存在{Fore.RESET}")
        return 1
    except (LookupError, UnicodeDecodeError) as e:
        print(f"{MSHead.Error} 编码错误: {Fore.RED}{e}{Fore.RESET}")
        return 1
    except Exception as e:
        print(f"{MSHead.Error} 未知错误: {Fore.RED}{e}{Fore.RESET}")
        return 1
