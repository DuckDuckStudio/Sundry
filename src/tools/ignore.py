...
def 追加忽略字段(检测程序路径: str, 忽略字段: str, 理由: str) -> bool | str:
    ...
    if 忽略字段 in "".join(lines[start_index:end_index]):
        print(f"{MSHead.Warning} {Fore.YELLOW}该忽略字段已存在{Fore.RESET}")
        return "skip"
...
def 编辑忽略字段(检测程序路径: str) -> None:
    print(f"{MSHead.Information} 检测脚本位于 {检测程序路径}")
    print(f"{MSHead.Information} 在默认程序中打开检测脚本...")
    # 使用默认程序打开检测程序并等待文件关闭
    open_file(检测程序路径)
    input(f"{MSHead.Message} 请在编辑完成后按 Enter 继续...")
...
def 创建拉取请求(分支名: str, owner: str, 忽略字段: str | None = None, 理由: str | None = None) -> str | int:
    ...
    if response.status_code == 201:
        ...
        print(f"{MSHead.Success} 拉取请求标记成功: {response.json()["html_url"]}")
        return response.json()["html_url"]
    else:
        print(f"{MSHead.Error} 拉取请求创建失败: {response.status_code} - {response.text}")
        return 0
...
def main(args: list[str]) -> int:
    ...
    # 同步
    try:
        print(f"{MSHead.Information} 开始同步...")
        subprocess.run(["git", "checkout", "main"], check=True)
        print(f"{MSHead.Information}     已签出到 main 分支")
        subprocess.run(["git", "pull"], check=True)
        print(f"{MSHead.Information}     已拉取远程修改")
    ...
    if (操作 != "list"):
        ...
        print(f"{MSHead.Information}     已签出新分支 - {新分支名}")
    print(f"{MSHead.Success} 同步完成")

    # ============ 操作入口 ============
    失败 = False
    if (操作 == "add"):
        print(f"{MSHead.Information} 开始追加...")
        ...
        print(f"{MSHead.Success} 已将忽略字段 {Fore.BLUE}{忽略字段}{Fore.RESET} 和理由 {Fore.BLUE}{理由}{Fore.RESET} 追加到检测程序的忽略列表中 ({Fore.YELLOW}{方式}{Fore.RESET})")

        print(f"{MSHead.Information} 开始提交和推送...")
        ...
        print(f"{MSHead.Success} 成功推送到远程")
        if not 创建拉取请求(新分支名, owner, 忽略字段):
            失败 = True
    elif (操作 == "remove"):
        print(f"{MSHead.Information} 开始移除...")
        方式 = 移除忽略字段(检测程序, 忽略字段, 新分支名)

        if not 方式:
            return 1

        print(f"{MSHead.Success} 已将忽略字段 {Fore.BLUE}{忽略字段}{Fore.RESET} 从检测程序的忽略列表中移除 ({Fore.YELLOW}{方式}{Fore.RESET})")
        ...
        print(f"{MSHead.Information} 现有忽略条目:")
        列出忽略字段(检测程序)
    elif (操作 == "edit"):
        编辑忽略字段(检测程序)
    ...
    # ============ 清理工作区 ============
    if (操作 != "list"):
        print(f"{MSHead.Information} 开始清理工作区...")
        subprocess.run(["git", "checkout", "main"], check=True)
        subprocess.run(["git", "branch", "-D", 新分支名], check=True)
        print(f"{MSHead.Success} 工作区清理完毕")
