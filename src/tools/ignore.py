...
def 追加忽略字段(检测程序路径: str, 忽略字段: str, 理由: str) -> bool | str:
    ...
    if 忽略字段 in "".join(lines[start_index:end_index]):
        print(f"{消息头.警告} {Fore.YELLOW}该忽略字段已存在{Fore.RESET}")
        return "skip"
...
def 编辑忽略字段(检测程序路径: str) -> None:
    print(f"{消息头.信息} 检测脚本位于 {检测程序路径}")
    print(f"{消息头.信息} 在默认程序中打开检测脚本...")
    # 使用默认程序打开检测程序并等待文件关闭
    open_file(检测程序路径)
    input(f"{消息头.消息} 请在编辑完成后按 Enter 继续...")
...
def 创建拉取请求(分支名: str, owner: str, 忽略字段: str | None = None, 理由: str | None = None) -> str | int:
    ...
    if response.status_code == 201:
        print(f"{消息头.成功} 拉取请求创建成功: {response.json()["html_url"]}")
        ...
        print(f"{消息头.成功} 拉取请求标记成功: {response.json()["html_url"]}")
...
def main(args: list[str]) -> int:
    ...
    if (操作 == "add"):
        ...
        print(f"{消息头.成功} 已将忽略字段 {Fore.BLUE}{忽略字段}{Fore.RESET} 和理由 {Fore.BLUE}{理由}{Fore.RESET} 追加到检测程序的忽略列表中 ({Fore.YELLOW}{方式}{Fore.RESET})")
